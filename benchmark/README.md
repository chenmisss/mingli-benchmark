...
4. **三池分离与来源切片**（私有集）：私有集记录分三个互斥池——**计分池**（年谱＋现代真实命例，唯一决定 top1 排名）、**诊断池**（古籍命例＋安慰剂对照题，**不计排名分**，只作背题/泄漏诊断）、其余。理由：①**古籍命例太典型**（几乎必在训练语料），若计入排名，"背过滴天髓"的系统白捡分、污染 top1——故摘出计分、只保留切片对比：古籍切片显著高于年谱/现代切片＝背原书指纹（一题不删，仍作答仍统计，只是不抬排名）。②**安慰剂对照题**（虚构命例＋引擎合法命盘＋随机金标；领题不可区分、选项同样重排、id 无标记）在随机金标上显著超机会水平（>2SE）即触发 `[BENCH ALERT]` 泄漏报警，其置信度＝对噪声的诚实度。能力边界诚实声明：诊断池可捉答案键泄漏与背书/出题指纹，不能捉联网查证型作弊（虚构人物查无此人，查证失败本身可识别对照）。
5. **客户端指纹与疑似标注**（2026-07-14 起）：注册与提交记录客户端指纹＝**加盐哈希**(IP＋UA)，只存哈希、原文仅入审计日志，绝不下发。榜单按公开规则自动标注 ⚠️疑似：①同源多应用（同指纹≥2 应用上榜＝马甲）；②短时跃升（同源/同应用 1 小时内成绩跃升≥15pp＝疑似以即时分数为 oracle 迭代刷分）；③人工标注（运营审计留痕，附理由）。标注不删除成绩、不隐藏行——数据照展示，读者自行判断。此前的历史提交无指纹，只做同应用内比较。
6. **官方集领题时窗**：私有集须先领题，且自首次领题起 `BENCH_PAPER_WINDOW_H`（默认 6h）内交卷，超时 409——堵"领题后无限期离线查证"。报警阈值等反作弊参数走环境变量，生产真值不可从开源代码读出。
