...
test('注册+列考集（public_dev/public_test 练习区；private 擂台榜·官方·可复测）', async () => {
  ...
  assert.equal(priv.official, true, 'private=官方擂台榜');
  assert.equal(priv.quota_per_app, 1, '擂台每家仅一次正式作答(一次定分防刷)');
});

test('远程 MCP：initialize + tools/list + 无通知流约定', async () => {
  const init = await j('POST', '/mcp', {
    jsonrpc: '2.0', id: 1, method: 'initialize',
    params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'test', version: '1' } },
  });
  assert.equal(init.status, 200);
  assert.equal(init.body.result.protocolVersion, '2025-06-18');
  assert.equal(init.body.result.serverInfo.name, 'sjms-benchmark');

  const tools = await j('POST', '/mcp', { jsonrpc: '2.0', id: 2, method: 'tools/list', params: {} });
  assert.equal(tools.status, 200);
  assert.deepEqual(tools.body.result.tools.map(t => t.name).sort(),
    ['get_exam_paper', 'get_leaderboard', 'get_task', 'list_exam_sets', 'register_team', 'submit_answers']);

  const get = await fetch(base + '/mcp');
  assert.equal(get.status, 405, '无 SSE 通知流时 GET 必须明确返回 405');
});
...
test('[擂台] private=官方即时出分榜(答案不可查故无需藏分)', () => {
  const lb = svc.leaderboard('private');
  assert.equal(lb.official, true);
  assert.equal(lb.results_revealed, true, '擂台即时出分(答案永不公开→无题可查,即时出分安全)');
  assert.ok('tracks' in lb, '擂台榜回逐条排名');
});
